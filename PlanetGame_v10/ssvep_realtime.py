...
GODOT_IP = "127.0.0.1"
GODOT_PORT = 4242
CMD_MAPPING = {10.0: "LEFT", 12.0: "RIGHT", 15.0: "FORWARD"}
COOLDOWN = 1.5 # Seconds between allowed commands to prevent accidental double-turning
...
class LSLStreamer(threading.Thread):
    ...
    def connect_godot(self):
        try:
            ...
            self.sock.connect((GODOT_IP, GODOT_PORT))
            print(f"Connected to Godot at {GODOT_IP}:{GODOT_PORT}")
            return True
        except Exception as e:
            print(f"Godot connection failed: {e}")
            self.sock = None
            return False

    def send_to_godot(self, command):
        if time.time() - self.last_sent_time < COOLDOWN:
            return
        ...
        if self.sock:
            try:
                ...
                print(f"SENT TO GODOT: {command}")
            except Exception as e:
                print(f"Godot send error: {e}")
                self.sock = None
    ...
    def run(self):
        self.callback("status", "Searching for LSL Stream...")
        self.connect_godot()
                ...
                if len(self.buffer) == self.buffer.maxlen:
                    ...
                    if len(self.history) == self.history.maxlen:
                        ...
                        if count > 2:
                            ...
                            if most_common_pred in CMD_MAPPING:
                                self.send_to_godot(CMD_MAPPING[most_common_pred])
