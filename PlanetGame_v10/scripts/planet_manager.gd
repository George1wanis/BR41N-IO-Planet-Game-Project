...
func create_planets_for_system(parent: Node3D, data_list: Array, system_idx: int, size_mult: float):
	...
	for i in range(data_list.size()):
		var data: Dictionary = data_list[i]
		...
		if data.get("has_rings"):
			var rings: MeshInstance3D = MeshInstance3D.new()
...
func cycle_system(dir: int):
	# Wrap around navigation for rows (systems)
	active_system_idx = (active_system_idx + dir + systems.size()) % systems.size()
	update_selection()
...
func update_selection(instant: bool = false):
	...
	var planets = planet_nodes[active_system_idx]
	var selected_planet: Node3D = planets[selected_planet_idx[active_system_idx]]
	...
	for i in range(systems.size()):
		...
		for p_node in planet_nodes[i]:
			var label: Label3D = p_node.get_node_or_null("Label3D")
			if label:
	...
	if planet_name_label and planet_desc_label:
		...
		var data: Dictionary = data_list[selected_planet_idx[active_system_idx]]
